...
1. 위쪽은 **품목 시트**다 — 엑셀처럼 한 화면에 전부 보이고, 머리글을 누르면 그 열로 정렬(다시 누르면 역순), 머리글과 번호 열은 스크롤해도 고정된다. 검색·품목구분 칩으로 거르고 `⬇ 엑셀(CSV)` 로 지금 보이는 그대로 내려받는다(엑셀에서 한글 안 깨짐). 열은 품번·품명·품목구분·분류·규격·기준단위·입고단위·사용기한·추적·조달·쓰이는 곳·자체 BOM·상태 13개. 행을 누르면 아래에 등록 폼이 열린다.
...
3. **사용기한(일)** 은 비우면 무기한 또는 분류 기본값(예: 화학품 365일) 상속. **입고단위** 는 사는 단위와 쓰는 단위가 다를 때만 — `1 BOX = 100 EA` 처럼 적고, 재고·BOM·로트는 언제나 기준단위로만 돈다.
