...
1. 위쪽은 **품목 시트**다 — 엑셀처럼 한 화면에 전부 보이고, 머리글을 누르면 그 열로 정렬(다시 누르면 역순), 머리글과 번호 열은 스크롤해도 고정된다. 검색·품목구분 칩으로 거르고 `⬇ 엑셀(CSV)` 로 지금 보이는 그대로 내려받는다(엑셀에서 한글 안 깨짐). 열은 품번·품명·품목구분·분류·규격·기준단위·입고단위·사용 시작일·사용 종료일·추적·조달·쓰이는 곳·자체 BOM·상태 14개이고, **탭이 좁아지면 덜 중요한 열부터 숨는다**(품번·품명·품목구분·상태는 항상 남고, 품번 열은 가로로 밀어도 붙어 있다). 행을 누르면 아래에 등록 폼이 열린다.
...
6. 행을 누르면 아래에 **역조회**가 나온다 — 그 거래처가 대는 품목(매입)과 사 가는 품목(매출)을 거래처 품번·단가·리드타임·최소발주·주거래까지 보여 주고, `품목 세부 →` 로 그 품목 화면으로 바로 넘어간다. 시트에도 `연결 품목` 건수 열이 있다. 연결을 **고치는** 곳은 품목 세부 한 곳이다.
