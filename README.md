...
public/
  ...
  js/items.js, css/items.css  📦 품목 등록 화면 (window.FWItems.mount — 품목구분·사용기한·입고단위 + BOM 연결, 인터페이스 §11)
  js/partners.js  🤝 거래처 화면 (window.FWPartners.mount — 매입·매출·매입/매출·기타, 인터페이스 §12; 스타일은 items.css 공용)
  js/itemdetail.js  품목 세부 화면 (window.FWItemDetail.mount — 식별정보 + 품목×거래처 연결, 인터페이스 §11.5)
...
- 세션 유지: 로그인은 브라우저에 12시간 유지된다. 관리자 콘솔 → [공장 맵으로], 테마·저사양 전환(새로고침), 재방문 시 로그인 없이 복귀하고
  (캐릭터 설정이 저장돼 있으면 곧바로 출근, 테마 전환은 같은 자리로), 콘솔은 세션 확인 동안 로그인 폼을 띄우지 않는다. 계정을 바꾸려면
  게임 상단 🚪 또는 콘솔 사이드바 [🚪 로그아웃](이 브라우저의 게임·콘솔 로그인을 모두 지움). 만료되면 로그인 화면으로 돌아간다.
  토큰에는 발급 당시 사번이 들어 있어, DB가 초기화되는 배포(Render)에서 같은 id의 다른 사용자로 인증되지 않는다(`GET /api/me`로 확인)
...
- **스키마**: `server/db.js` 가 기동 때 `docs/4m/ddl-v1.sql` 을 **파일로 읽어** 문 단위로 실행한다(코드에 복사하지 않음 — 원천 하나). 실패하면 `[db] ddl-v1.sql 실행 실패 — 문 #n/85 "…": 오류` 를 남기고 서버는 뜬다.
  트리 3축: ① 품목 분류 `mat_class`(자기참조) ② 제품 구조 `bom_header`+`bom_line`(부모-자식 간선, 유효일자·대체품·팬텀) ③ 로트 `mat_lot`(분할)+`lot_genealogy`(투입 계보). BOM↔BOP 는 `process_material`(공정별 IN/OUT).
  무결성은 DB 트리거가 지킨다 — 순환 참조(R-1), 유효기간 겹침(R-9/9b), 단위·소수(R-7/8), 삭제 금지(R-11: 상태 OBSOLETE 로), 계보 순환(R-13), 투입 순서(R-22) 등. 점검 뷰 `v_chk_*` + `v_chk_summary`.
- **이행 (기존 DB, 기동 시 자동·멱등)**: `parts`·`process_inputs` 표에 데이터가 있고 `item` 이 비어 있으면 [1] 적재기가 `cleansing-v1.json` 대로 채우고 → [2] `expected`·`v_chk_summary` 를 대조해(어긋나면 롤백, legacy 유지) → [3] 두 표를 `parts_legacy`·`process_inputs_legacy` 로 이름 바꾸고 **같은 이름의 호환 뷰**(`v_parts_compat`·`v_process_inputs_compat`)를 만든다.
  그래서 `equipment:detail.process.inputs[]`·`GET /api/admin/bop`·분석의 조회 쿼리는 무수정이다(`qty` = 완성품 1대당 전개값, 뷰가 `rowid` 컬럼을 내서 `ORDER BY rowid` 유지). legacy 표는 지우지 않는다. 되돌리기는 [운영 가이드 §2](docs/운영전환-가이드.md#2-가용성-nfr-04).
  **첫 기동 전에 `node scripts/backup.js` 로 백업**할 것(이 PC: `fw-backup-20260913-115025` 뒤 이행, item 60·bom_line 59·IN 46·OUT 24, legacy 56/36행 보존).
- **적재기** (`server/materials.js` `loadMaterials`): uom 5 → uom_conv 1 → mat_class 24 → item 60(원천 product 1 + 서브어셈블리 9 + 단품 47 + 신규 3; 승인 전 신규 품번은 `TMP-SC1010P`·`TMP-FS7023`·`TMP-PK0010`) → bom_header 11 → bom_line 59(복합 부모 `HA-3000/EX-5000` 분리, 자기참조 2건 정제, SC-1011 재배치) → 산출 OUT 24 → 투입 IN 46(BOP 표기 순서 = 상태창 정렬). 전부 UPSERT(재실행 안전, 삭제 없음).
...
- **관리자 콘솔 🧩 자재 탭 사용법** (서지안, `public/js/materials.js` `window.FWMaterials.mount(container, api)` + `css/materials.css`):
  ...
  2. 좌측 **품목 분류 트리**(클릭 = 하위 분류 포함 필터) → 가운데 4개 섹션. **품목 · BOM 트리 · 로트 계보 세 섹션이 모두 마스터 | 디테일 2단**이다(품목 목록|상세, BOM 트리|라인 편집·헤더, 로트 표|정·역 계보). 기본은 정확히 반반이고 섹션마다 위치·비율을 따로 기억한다. 공정 연결은 위 드롭다운이 마스터라 나누지 않았다: **품목**(검색·종류·상태, 행을 누르면 오른쪽에 속성·산출/투입 공정·where-used·헤더·편집) · **BOM 트리**(부모 품번·기준일 as-of·깊이 → 정전개, 팬텀 점선·⚠ 미배정·TMP 배지·IN/◀ 산출 공정 칩, 말단 합계 + 기준단위 환산; 행 클릭 = 라인 편집 폼, [+ 헤더]·상태 전이) · **공정 연결**(공정 선택 → IN/OUT/미배정, 편집 모드에서 순서·출고 방식·미배정 연결) · **로트 계보**(품목/검색 → 로트 표 → 행 클릭 → 역방향(밀시트까지)/정방향(완성품 시리얼까지) 트리, [투입 기록]·[+ 로트]).
...
🧩 자재 탭이 **있는 기준을 보는** 화면이라면, 이 탭은 **품번을 새로 만들어 BOM 에 붙이는** 화면이다. 현장 담당자가 쓰라고 필수를 세 칸으로 줄였다.

1. 위쪽은 **품목 시트**, 행을 누르면 **오른쪽(기본)이나 아래**에 등록 폼이 열린다 — 도구줄의 `상세 [아래|오른쪽]` 토글로 고르고 그 선택은 브라우저에 남는다(화면별로 따로). 오른쪽에 두면 **화면을 정확히 반반**으로 나누고, 가운데 경계를 끌어 비율을 바꿀 수 있다(25~75%, 두 번 누르면 다시 반반, 그 값도 기억된다). 폭이 모자라면 오른쪽을 골라 뒀어도 아래로 내려간다. 시트의 열 숨김은 **시트가 실제로 쓰는 칸 폭**을 보므로 반반으로 나눠도 가로 스크롤이 생기지 않는다(아래 1629px 15열 / 반반 799px 8열). 시트 자체는 — 엑셀처럼 한 화면에 전부 보이고, 머리글을 누르면 그 열로 정렬(다시 누르면 역순), 머리글과 번호 열은 스크롤해도 고정된다. 검색·품목구분 칩으로 거르고 `⬇ 엑셀(CSV)` 로 지금 보이는 그대로 내려받는다(엑셀에서 한글 안 깨짐). 열은 품번·품명·품목구분·분류·규격·기준단위·입고단위·사용 시작일·사용 종료일·추적·조달·쓰이는 곳·자체 BOM·상태 14개이고, **탭이 좁아지면 덜 중요한 열부터 숨는다**(품번·품명·품목구분·상태는 항상 남고, 품번 열은 가로로 밀어도 붙어 있다). 행을 누르면 아래에 등록 폼이 열린다.
2. **품번 · 품명 · 품목구분** 세 칸만 채우면 등록된다. 품목구분 9종(제품·상품·반제품·재공품·부품·원자재·부자재·소모품·포장재)을 고르면 분류·조달구분·추적단위·사용기한 기본값이 따라온다 — 직접 건드린 칸은 덮어쓰지 않는다. 구분은 화면 표시축(`item.item_group`)이고 전개·소요량·점검은 종전대로 `item_type` 으로 계산한다.
3. **사용기간**은 시작일 ~ 종료일 두 날짜로 적는다. 종료일을 비우면 무기한이고, 분류 기본값(예: 화학품 365일)이 있으면 시작일 + 그 일수를 계산해 `*` 와 함께 보여 준다(저장은 안 한다). 기간의 길이가 곧 로트 보관 일수다 — 로트 유효일 = 입고일 + 그 일수. **입고단위** 는 사는 단위와 쓰는 단위가 다를 때만 — `1 BOX = 100 EA` 처럼 적고, 재고·BOM·로트는 언제나 기준단위로만 돈다.
4. 아래 **BOM 연결** 에서 상위 품목·소요량·투입 공정을 지정하면 `POST bom-attach` 가 BOM 라인 1행(+ 선택 시 BOP 투입 1행)을 만든다. 운영 중인 BOM 에도 붙는다 — 라인 유효일이 오늘부터라 과거 전개 결과는 그대로다. 뺄 때는 DRAFT 면 삭제, 운영이면 유효일로 끊는다.
5. 새 품목은 **작성중(DRAFT)** 으로 시작해 BOM 에 붙는 순간 **사용(ACTIVE)** 으로 올라간다 — 쓰이는 곳 없는 ACTIVE 품목은 점검 R-4 에 고아로 잡히기 때문이다. 단독으로 쓰는 품목이면 직접 사용으로 바꾸면 되고, 그때 화면이 경고를 보여 준다.
6. 순환(R-1)·공정 순서(R-22)·중복 같은 규칙 위반은 서버가 400 으로 거부하고, 화면은 그 뜻을 현장 말로 풀어 원문과 함께 보여 준다.

계약은 `docs/team/인터페이스.md` **§11**. `item` 에 NULL 허용 컬럼 4개(`item_group`·`shelf_life_days`·`in_uom`·`in_qty`)가 늘었고 DDL 문 개수는 85 그대로다 — 기존 DB 는 부팅 시 `ALTER TABLE` 로 따라온다.

### 품목 세부 탭 (스프린트 4) — 품목 등록의 하위 메뉴

품번 하나를 골라 살을 붙이는 화면이다. 📦 품목 등록이 "만드는" 곳이라면 여기는 "채우는" 곳이다.

1. **식별정보** — 도면번호·GTIN(바코드)·참조 단가·이미지. 참조 단가는 말 그대로 참조값이고 원가 전개에 쓰지 않는다. 실제 매입가는 아래 거래처별 단가다.
2. **매입처·매출처** — 한 품목에 공급사가 여럿인 것이 정상이라 표로 붙인다. 거래처마다 **거래처 품번·단가·통화·리드타임·발주단위·최소발주량**을 따로 둔다. 거래처의 거래구분과 역할이 맞아야 붙고(매출 전용 거래처를 매입처로 붙이면 400), **주거래처는 품목·역할당 한 곳**이라 새로 지정하면 이전 주거래처가 자동으로 내려간다.
3. **단위 규칙**: 적용 순서는 `거래처별 발주단위 > 품목의 기본 입고단위 > 기준단위(낱개)`다. 공급사마다 포장이 다른 게 보통이라 거래처 쪽이 이긴다. 물려받은 값은 표에 `*`로 표시된다. **최소발주는 발주단위 기준**이고 괄호로 기준단위 환산을 함께 보여 준다(`2 CASE (1,000 EA)`). 재고·BOM·로트는 어느 경우에도 기준단위로만 돈다.
4. **BOM 요약**은 읽기 전용이다 — 구조를 고치는 곳은 📦 품목 등록의 BOM 연결과 🧩 자재 탭이다.

표는 `item_partner`(`server/db.js`, `item`·`partners` 를 둘 다 참조하므로 자재 DDL 실행 뒤에 만든다). 계약은 **§11.5**.

**좌측 메뉴**는 이 스프린트에서 평면 탭 → **운영 · 기준정보 · 설정** 3묶음으로 바뀌었다. 묶음 제목을 누르면 접히고(브라우저에 기억된다), `품목 세부`는 `품목 등록` 아래 들여쓴 하위 항목이다. 시트가 있는 탭은 본문 폭 제한을 1100px → 1700px 로 푼다 — 그러지 않으면 넓은 화면에서도 열이 접힌 채로만 보였다.

### 🤝 거래처 탭 (스프린트 4)

매입처와 매출처를 한 표에서 관리한다. 품목 등록 탭과 같은 시트 구조(고정 머리글·정렬·CSV·폭 반응형)다.

1. 거래구분은 **매입 · 매출 · 매입·매출 · 기타** 넷. 사서 팔기도 하는 곳이 흔해서 `매입·매출`을 따로 뒀고, **매입이나 매출로 거르면 `매입·매출` 거래처도 함께 나온다** — 실제로 그 역할을 하기 때문이다. 칩의 건수도 같은 규칙으로 센다.
2. 필수는 **거래처명 · 거래구분** 두 칸. 코드를 비우고 저장하면 `P0001`부터 비어 있는 번호가 자동으로 붙는다(사내 코드가 있으면 그대로 써도 된다).
3. **사업자등록번호는 국세청 검증식까지 확인**한다. 통과하면 `000-00-00000`으로 정규화해 저장하고, 같은 번호가 이미 있으면 막는다. 이메일 형식도 본다.
4. 그 밖에 대표자·업태·종목·연락처·주소·담당자(상대 쪽 창구)·결제조건·통화를 받는다.
5. 거래처는 **지우지 않는다**. 안 쓰면 상태를 `거래종료`로 바꾼다 — 지우면 과거 전표가 거래처를 잃는다. 삭제 API 는 405로 막혀 있다.
6. 행을 누르면 상세와 함께 **역조회**가 나온다(위치는 품목 등록과 같은 토글로 아래/오른쪽 선택) — 그 거래처가 대는 품목(매입)과 사 가는 품목(매출)을 거래처 품번·단가·리드타임·최소발주·주거래까지 보여 주고, `품목 세부 →` 로 그 품목 화면으로 바로 넘어간다. 시트에도 `연결 품목` 건수 열이 있다. 연결을 **고치는** 곳은 품목 세부 한 곳이다.

계약은 `docs/team/인터페이스.md` **§12**. 표는 `server/db.js` 의 `partners`(자재 기준 `ddl-v1.sql` 은 건드리지 않는다). 품목과의 연결(주거래처·공급사 품번·단가)은 다음 단계인 **품목 세부 등록** 화면에서 `item_partner` 로 붙인다.
