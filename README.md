...
1. **품번 · 품명 · 품목구분** 세 칸만 채우면 등록된다. 품목구분 9종(제품·상품·반제품·재공품·부품·원자재·부자재·소모품·포장재)을 고르면 분류·조달구분·추적단위·사용기한 기본값이 따라온다 — 직접 건드린 칸은 덮어쓰지 않는다. 구분은 화면 표시축(`item.item_group`)이고 전개·소요량·점검은 종전대로 `item_type` 으로 계산한다.
2. **사용기한(일)** 은 비우면 무기한 또는 분류 기본값(예: 화학품 365일) 상속. **입고단위** 는 사는 단위와 쓰는 단위가 다를 때만 — `1 BOX = 100 EA` 처럼 적고, 재고·BOM·로트는 언제나 기준단위로만 돈다.
3. 아래 **BOM 연결** 에서 상위 품목·소요량·투입 공정을 지정하면 `POST bom-attach` 가 BOM 라인 1행(+ 선택 시 BOP 투입 1행)을 만든다. 운영 중인 BOM 에도 붙는다 — 라인 유효일이 오늘부터라 과거 전개 결과는 그대로다. 뺄 때는 DRAFT 면 삭제, 운영이면 유효일로 끊는다.
4. 새 품목은 **작성중(DRAFT)** 으로 시작해 BOM 에 붙는 순간 **사용(ACTIVE)** 으로 올라간다 — 쓰이는 곳 없는 ACTIVE 품목은 점검 R-4 에 고아로 잡히기 때문이다. 단독으로 쓰는 품목이면 직접 사용으로 바꾸면 되고, 그때 화면이 경고를 보여 준다.
5. 순환(R-1)·공정 순서(R-22)·중복 같은 규칙 위반은 서버가 400 으로 거부하고, 화면은 그 뜻을 현장 말로 풀어 원문과 함께 보여 준다.
