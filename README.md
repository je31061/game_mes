...
- **관리자 콘솔 🧩 자재 탭 사용법** (서지안, `public/js/materials.js` `window.FWMaterials.mount(container, api)` + `css/materials.css`):
  ...
  2. 좌측 **품목 분류 트리**(클릭 = 하위 분류 포함 필터) → 가운데 4개 섹션: **품목**(검색·종류·상태 — **목록 | 상세 2단**이라 행을 누르면 오른쪽에 속성·산출/투입 공정·where-used·헤더·편집이 바로 뜬다. 오른쪽은 스크롤을 따라오고, 탭이 좁으면 예전처럼 목록 아래로 내려간다) · **BOM 트리**(부모 품번·기준일 as-of·깊이 → 정전개, 팬텀 점선·⚠ 미배정·TMP 배지·IN/◀ 산출 공정 칩, 말단 합계 + 기준단위 환산; 행 클릭 = 라인 편집 폼, [+ 헤더]·상태 전이) · **공정 연결**(공정 선택 → IN/OUT/미배정, 편집 모드에서 순서·출고 방식·미배정 연결) · **로트 계보**(품목/검색 → 로트 표 → 행 클릭 → 역방향(밀시트까지)/정방향(완성품 시리얼까지) 트리, [투입 기록]·[+ 로트]).
