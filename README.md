...
2. **매입처·매출처** — 한 품목에 공급사가 여럿인 것이 정상이라 표로 붙인다. 거래처마다 **거래처 품번·단가·통화·리드타임·최소발주량**을 따로 둔다. 거래처의 거래구분과 역할이 맞아야 붙고(매출 전용 거래처를 매입처로 붙이면 400), **주거래처는 품목·역할당 한 곳**이라 새로 지정하면 이전 주거래처가 자동으로 내려간다.
3. **BOM 요약**은 읽기 전용이다 — 구조를 고치는 곳은 📦 품목 등록의 BOM 연결과 🧩 자재 탭이다.
